...
```{r}
mms2 <- lme(fixed = HighIncome ~ FamilyType, method = "ML", data = acs_fit, random =~ FamilyType | HouseCosts)
summary(mms2)
acs_fit$linest_HighIncome <- ifelse(mms2$fit > 250000,1,0)
mms2_aic <- AIC(mms2)
mms2_aic
```
```{r}
