...
summary(linear_mod1)
acs_fit$linest_HighIncome <- ifelse(linear_mod1$fit > 250000,1,0)
linear_cm1 <- confusionMatrix(as.factor(acs_fit$linest_HighIncome), as.factor(acs_fit$HighIncome), positive = "1")
linear_cm1
```
...
summary(mms2)
```
Simple linear regressions, and chose this one. I based my decision on sprintf("Logistic model 4: Predicted Accuracy = %.3f Predicted Sensitivity = %.3f", log_cm4$overall['Accuracy'], 
        log_cm4$byClass['Sensitivity'])R-squared and changes in adjusted R-
...
library(nlme)
mms1 <- lme(fixed = FamilyIncome ~ 1, method = "ML", data = acs_fit, random =~ 1 | HouseCosts)
summary(mms1)acs_fit$linest_HighIncome <- ifelse(linear_mod2$fit > 250000,1,0)
linear_cm2 <- confusionMatrix(as.factor(acs_fit$linest_HighIncome), as.factor(acs_fit$HighIncome), positive = "1")
linear_cm2
```
```{r}
...
sprintf("Linear model 1:   Predicted Accuracy = %.3f Predicted Sensitivity = %.3f adj R-squared = %.3f", 
        linear_cm1$overall['Accuracy'], linear_cm1$byClass['Sensitivity'], 
        summary(linear_mod1)$adj.r.squared['Adjusted R-squared'])

sprintf("Linear model 2:   Predicted Accuracy = %.3f Predicted Sensitivity = %.3f",linear_cm2$overall['Accuracy'], 
        linear_cm2$byClass['Sensitivity'])
sprintf("Logistic model 1: Predicted Accuracy = %.3f Predicted Sensitivity = %.3f", log_cm1$overall['Accuracy'], 
        log_cm1$byClass['Sensitivity'])
sprintf("Logistic model 2: Predicted Accuracy = %.3f Predicted Sensitivity = %.3f", log_cm2$overall['Accuracy'], 
        log_cm2$byClass['Sensitivity'])
sprintf("Logistic model 3: Predicted Accuracy = %.3f Predicted Sensitivity = %.3f", log_cm3$overall['Accuracy'], 
        log_cm3$byClass['Sensitivity'])
sprintf("Logistic model 4: Predicted Accuracy = %.3f Predicted Sensitivity = %.3f", log_cm4$overall['Accuracy'], 
        log_cm4$byClass['Sensitivity'])
sprintf("Logistic model 5: Predicted Accuracy = %.3f Predicted Sensitivity = %.3f", log_cm5$overall['Accuracy'], 
        log_cm5$byClass['Sensitivity'])
