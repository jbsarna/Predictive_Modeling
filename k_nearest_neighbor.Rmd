...
a) This chunk of code is the same for all pages in this R-project
b) It is included in each rmd file so that you can examine each predictive method individually
...
# k-nearest neighbor
a) k-nearest neighbor can only take numerical data
b) It is recommended (in most all cases) to normalize the data set
...
1) I would choose knn model 6 because highest (tested) accuracy with a "relatively" low sensitivity
...
3) One thing to be carefule of is that the higher the k value the more complex the "outline" of the determinations. This 
    could lead to a "garbage" model (as I call it)
