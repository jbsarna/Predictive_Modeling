...
```
```{r}
acs_norm3$groups <- cutree(pfit2, k=5)
```
...
gam_mod_cluster2 <- gam(HighIncome ~ s(acs.Insurance) + s(acs.HouseCosts) + s(acs.ElectricBill) + 
                         ...
                         groups, data = acs_norm3, family=binomial(link="logit"))
...
acs_norm3$gam_cluster2_HighIncome <- ifelse(gam_mod_cluster2$fit > .5 ,1,0)

gam_cluster_cm2 <- confusionMatrix(as.factor(acs_norm3$gam_cluster2_HighIncome), as.factor(acs_norm3$HighIncome),
                                  positive = "1")
