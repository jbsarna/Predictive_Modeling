...
```
```{r}
rand_bag1 <- randomForest(FamilyIncome ~ Insurance + ElectricBill + HouseCosts + NumBedrooms , data=acs_test, mtry=4,
...
```{r}
plot(rand_bag1)
#bag1_pred <- predict(rand_bag1, acs_fit, type="class" )
#acs_fit$HighIncome_bag <- ifelse(bag1_pred$predicted > 250000,1,0)
#bag_cm1 <- confusionMatrix(bag1_pred, acs_fit$HighIncome_bag, positive = "1")
#bag_cm1
```

#stopped here!
```{r}
acs_fit$rand_tree1_pred <- predict(randtree_mod1, acs_fit, type="class" )
acs_fit$rand_tree1_pred <- ifelse(acs_fit$rand_tree1_pred > 250000,1,0)
rand_tree_cm1_pred <- confusionMatrix(acs_fit$rand_tree1_pred, acs_fit$HighIncome, positive = "1")
#accuracyMeasures(predict(randtree_mod1, newdata=acs_fit, acs_fit$HighIncome == 'FamilyIncome', name='random forest, train')
```